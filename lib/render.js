...
function getBiscuit(data, meta) {
  const componentName = _.get(meta, 'locals.params.name');
  const biscuitName = _.get(meta, 'locals.query.b');
  const { biscuits } = require('./setup');

  return {
    name: biscuitName,
    componentName,
    data: _.get(biscuits, `${componentName}._biscuits.${biscuitName}`, data)
  };
}

function makeState(data, meta) {
  const { _refs, components } = getIndices(meta._layoutRef || meta._ref, data);
  const biscuitData = getBiscuit(data, meta);

  return Object.assign(
    {
      ref: meta._ref,
      refs: _refs,
      initialBiscuit: biscuitData,
